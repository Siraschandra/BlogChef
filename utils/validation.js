...
export const storePostValidation = [
  check('post.title').notEmpty().withMessage("'title' should not be empty").escape(),
  check('post.content').notEmpty().withMessage('content field should not be empty').escape(),
  check('post.user').notEmpty().withMessage("'user' should not be empty").isString(),
  manageErrors((error, req, res) => res.status(422).json({ error })),
];
...
export const jwtValidation = [check('token').isJWT().notEmpty(), manageErrors((error, req, res) => res.status(422).json({ error }))];
